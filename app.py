...
try:
    ...
    logger.info("OCREngineManager导入成功")
except ImportError as e:
    logger.error(f"OCREngineManager导入失败: {str(e)}")
    OCREngineManager = None
...
try:
    ...
    logger.info("优化配置模块导入成功")
except ImportError as e:
    logger.error(f"优化配置模块导入失败: {str(e)}")
    # 提供默认实现
...
app.config['USE_GPU'] = False  # 禁用GPU，使用CPU
...
app.config['ENABLE_MIXED_PRECISION'] = False  # 禁用混合精度（CPU模式不需要）
...
OCR_LANGUAGES = {
    ...
    'multi_asian': {'name': '亚洲多语言', 'description': '中文+日文+韩文+英文'},
    'multi_european': {'name': '欧洲多语言', 'description': '英文+法文+德文+西班牙文+俄文'},
    'multi_all': {'name': '全语言混合', 'description': '支持所有主要语言的混合识别'},
    'smart_separate': {'name': '智能分离识别', 'description': '分别用每种语言识别后智能整合，获得最高质量'},
    'custom': {'name': '自定义组合', 'description': '用户自定义语言组合'},
}

# 可选择的单独语言列表（用于智能分离识别）
AVAILABLE_LANGUAGES = {
    'chinese': {'name': '中文', 'code': 'chinese'},
    'english': {'name': '英文', 'code': 'english'},
    'japanese': {'name': '日文', 'code': 'japanese'},
    'korean': {'name': '韩文', 'code': 'korean'},
    'french': {'name': '法文', 'code': 'french'},
    'german': {'name': '德文', 'code': 'german'},
    'spanish': {'name': '西班牙文', 'code': 'spanish'},
    'russian': {'name': '俄文', 'code': 'russian'},
    'arabic': {'name': '阿拉伯文', 'code': 'arabic'},
}
...
def initialize_ocr_manager():
    ...
    with ocr_manager_lock:
        if ocr_manager is None:
            try:
                if OCREngineManager is None:
                    logger.warning("OCREngineManager类未导入，跳过OCR引擎管理器初始化")
                    return
...
def convert_chinese_languages_to_codes(language_string):
    """将中文语言名称转换为语言代码"""
    if not language_string:
        return language_string
    
    # 如果已经是代码格式（包含下划线或全是英文），直接返回
    if '_' in language_string or language_string.isascii():
        return language_string
    ...
    for lang in languages:
        # 查找对应的语言代码
        code = CHINESE_TO_CODE_MAP.get(lang)
        if code:
            converted_languages.append(code)
        else:
            # 如果找不到对应的代码，保持原样
            converted_languages.append(lang)
            logger.warning(f"未找到语言 '{lang}' 对应的代码，保持原样")
    
    result = '+'.join(converted_languages)
    logger.info(f"语言转换: '{language_string}' -> '{result}'")
    return result
...
def upload_file():
    ...
    if request.form.get('use_custom_config') == 'true':
        ...
        # 处理智能分离识别
        if job_config['ocr_language'] == 'smart_separate':
            selected_languages = request.form.getlist('selected_languages')
            if selected_languages:
                job_config['ocr_language'] = 'separate:' + ','.join(selected_languages)
        
        # 处理自定义语言组合
        elif job_config['ocr_language'] == 'custom':
            custom_languages = request.form.get('custom_languages', '')
            if custom_languages:
                # 转换中文语言名称为代码
                job_config['ocr_language'] = convert_chinese_languages_to_codes(custom_languages)
    else:
        # 使用全局配置
...
def reprocess_job(job_id):
    ...
    if 'file_path' not in job or not os.path.exists(job['file_path']):
        return jsonify({'success': False, 'error': '原始文件不存在，无法重新处理'}), 404
    ...
    try:
        ...
        # 使用请求中的配置或当前全局配置
        new_config['ocr_engine'] = data.get('ocr_engine', current_config['ocr_engine'])
        new_config['ocr_level'] = data.get('ocr_level', current_config['ocr_level'])
        new_config['ocr_language'] = data.get('ocr_language', current_config['ocr_language'])
        new_config['extract_table_format'] = data.get('extract_table_format', current_config['extract_table_format'])
        new_config['extract_formula'] = data.get('extract_formula', current_config['extract_formula'])
        new_config['ollama_model'] = data.get('ollama_model', current_config['ollama_model'])
        
        # 如果是自定义语言组合
        if new_config['ocr_language'] == 'custom':
            custom_languages = data.get('custom_languages', '')
            if custom_languages:
                # 转换中文语言名称为代码
                new_config['ocr_language'] = convert_chinese_languages_to_codes(custom_languages)
        
        # 如果是智能分离识别
        elif new_config['ocr_language'] == 'smart_separate':
            selected_languages = data.get('selected_languages', [])
            if selected_languages:
                # 格式化为 separate:lang1,lang2,lang3
                new_config['ocr_language'] = 'separate:' + ','.join(selected_languages)
        ...
        jobs[new_job_id] = {
            ...
            'created_at': time.time(),
            'config': new_config,
            'reprocessed_from': job_id,  # 标记这是重新处理的作业
            'reprocess_reason': data.get('reason', '用户重新处理')
        }

        # 将新作业添加到队列
        with queue_lock:
            job_queue.append(new_job_id)
        ...
        logger.info(f"作业 {job_id} 已重新排队处理，新作业ID: {new_job_id}")
        return jsonify({
            'success': True, 
            'message': '作业已重新排队处理',
        ...
        logger.error(f"重新处理作业 {job_id} 失败: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
if __name__ == '__main__':
    ...
    # 支持外网访问
    import os
    host = os.environ.get('HOST', '0.0.0.0')  # 默认绑定所有网络接口
    port = int(os.environ.get('PORT', 5000))   # 默认端口5000
    debug = os.environ.get('DEBUG', 'True').lower() == 'true'
    
    logger.info(f"Starting server on {host}:{port}")
    logger.info(f"Debug mode: {debug}")
    logger.info("Server will be accessible from external networks")
